...
export async function ensureInitialData() {
  ...
  try {
    // Fast exit if database already has data
    const existingAdminCount = await prisma.admin.count().catch(() => 0);
    ...
    const defaultCase = await prisma.group.upsert({
      ...
      create: {
        ...
        winText: `🎉 Лорд Блэквуд отомщен! Дворецкий Джеймс Спенсер сознался под тяжестью улик.`,
        loseText: `❌ Настоящий убийца остался на свободе. Невиновный взят под стражу.`,
      },
    }).catch(() => null);
